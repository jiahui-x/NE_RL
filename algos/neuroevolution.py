...
class SSNE:
	...
	def crossover_inplace(self, gene1, gene2):
		...
		keys1 =  list(gene1.qnet.state_dict())
		keys2 = list(gene2.qnet.state_dict())

		for key in keys1:
			if key not in keys2: continue

			# References to the variable tensors
			W1 = gene1.qnet.state_dict()[key]
			W2 = gene2.qnet.state_dict()[key]
	...
	def mutate_inplace(self, gene):
		...
		num_params = len(list(gene.qnet.parameters()))
		ssne_probabilities = np.random.uniform(0, 1, num_params) * 2

		for i, param in enumerate(gene.qnet.parameters()):  # Mutate each param
	...
	def reset_genome(self, gene):
		...
		"""
		for param in (gene.qnet.parameters()):
	...
	def epoch(self, gen, pop, fitness_evals, migration):
		...
		#Migration Tracker
		if len(self.rl_policy) >0:  # RL Transfer happened
			for policy in self.rl_policy:
				self.selection_stats['total'] += 1.0
				if policy in elitist_index:
					self.selection_stats['elite'] += 1.0
				elif policy in offsprings:
					self.selection_stats['selected'] += 1.0
				elif policy in unselects:
		...
		for policy in migration:
			...
			pop[replacee] = copy.copy(policy)
			self.rl_policy.append(replacee)
		...
		for i, j in zip(unselects[0::2], unselects[1::2]):
			...
			pop[i] = copy.copy(pop[off_i])
			pop[j] = copy.copy(pop[off_j])
		...
		for net_i in range(len(pop)):
			if net_i not in new_elitists:  # Spare the new elitists
				# if random.random() < (self.args.mutation_prob-0.1) * gen/self.args.max_gen+0.1:
